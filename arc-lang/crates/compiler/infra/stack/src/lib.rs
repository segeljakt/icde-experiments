...
// A scope has a kind and can contain expr and type declarations that exist in separate namespaces.
#[derive(Clone, Debug)]
...
// A namespace is a list of bindings from names to declarations.
#[derive(Clone, Debug)]
...
impl<D> Namespace<D> {
    ...
    pub fn iter_mut(&mut self) -> impl Iterator<Item = &mut (Name, D)> {
        self.0.iter_mut().rev()
    }
}
