...
pub struct Compiler {
    ...
    pub(crate) ctx5: hir_ssa::context::Context,
    pub(crate) ctx4: hir_flatten::context::Context,
    pub(crate) ctx6: hir_monomorphise::context::Context,
    pub(crate) ctx7: hir_interpreter::context::Context,
}
...
impl Compiler {
    ...
    pub fn new(config: Config, logger: Logger) -> Self {
        ...
        this.ctx6.diagnostics.backtrace = this.config.show.backtrace;
        this.ctx7.ctx7.diagnostics.backtrace = this.config.show.backtrace;
        this.ctx7.ctx8.diagnostics.backtrace = this.config.show.backtrace;
        this.ctx7.ctx9.diagnostics.backtrace = this.config.show.backtrace;
        ...
        this.ctx6.diagnostics.failfast = this.config.failfast;
        this.ctx7.ctx7.diagnostics.failfast = this.config.failfast;
        this.ctx7.ctx8.diagnostics.failfast = this.config.failfast;
        this.ctx7.ctx9.diagnostics.failfast = this.config.failfast;
        this
    }
    ...
    pub fn lambda_lift(&mut self, ss: Vector<hir::Stmt>) -> Vector<hir::Stmt> {
        let ss = hir_lambda_lift::process(&mut self.ctx2, ss);
        if self.config.show.lifted {
            if !ss.is_empty() {
                self.show_hir(&ss);
            }
            return Vector::new();
    ...
    pub fn flatten(&mut self, ss: Vector<hir::Stmt>) -> Vector<hir::Stmt> {
        let ss = hir_flatten::process(&mut self.ctx4, ss);
        if self.config.show.flattened {
            if !ss.is_empty() {
                self.show_hir(&ss);
            }
            return Vector::new();
    ...
    pub fn ssa(&mut self, ss: Vector<hir::Stmt>) -> Vector<hir::Stmt> {
        let ss = hir_ssa::process(&mut self.ctx5, ss);
        if self.config.show.ssa {
            if !ss.is_empty() {
                self.show_hir(&ss);
            }
            return Vector::new();
    ...
    pub fn monomorphise(&mut self, ss: Vector<hir::Stmt>) -> Vector<hir::Stmt> {
        let ss = hir_monomorphise::process(&mut self.ctx6, ss);
    ...
    pub fn interpret(&mut self, ss: Vector<hir::Stmt>) {
        hir_interpreter::process(&mut self.ctx7, ss)
    }

    pub fn clear_caches(&mut self) {
        self.ctx7.ctx10.clear_caches();
    }

    pub fn show_caches(&mut self) {
        self.ctx7.ctx10.show_caches();
    }

    pub fn stmts(&self) -> Vector<Stmt> {
        self.ctx6.stmts.clone()
    }
    ...
    pub fn has_errors(&self) -> bool {
        self.ctx0.diagnostics.has_errors()
            ...
            || self.ctx6.diagnostics.has_errors()
            || self.ctx7.ctx8.diagnostics.has_errors()
    }
    ...
    pub fn emit_errors(&mut self) {
        ...
        self.ctx6.diagnostics.emit_errors(sources, opt);
        self.ctx7.ctx7.diagnostics.emit_errors(sources, opt);
        self.ctx7.ctx8.diagnostics.emit_errors(sources, opt);
        self.ctx7.ctx9.diagnostics.emit_errors(sources, opt);
        self.ctx7.ctx10.diagnostics.emit_errors(sources, opt);
    }
    ...
    pub fn clear(&mut self) {
        ...
        self.ctx7 = Default::default();
        self.ctx7.ctx7 = Default::default();
        self.ctx7.ctx8 = Default::default();
        self.ctx7.ctx9 = Default::default();
        self.ctx7.ctx10 = Default::default();
    }
