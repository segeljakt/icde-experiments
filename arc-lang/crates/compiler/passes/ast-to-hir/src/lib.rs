...
pub fn lower_stmt(ctx: &mut Context, s: ast::Stmt) -> Stmt {
    ...
    match s.kind {
        ...
        ast::SMod(m, x, ss) => {
            ...
            // let ss = ss.mapm(ctx, lower_stmt);
            // SMod(m, x, ss).with(info)
        }
        ast::SUse(m, xs, x) => {
            todo!()
        }
        ast::SExpr(e) => {
            let e = lower_expr(ctx, e);
            SVar(PIgnore.with(e.t.clone(), info), e).with(info)
        }
...
fn lower_pattern(ctx: &mut Context, p: ast::Pattern, m: bool) -> Pattern {
    ...
    match (*p.kind).clone() {
        ...
        ast::PVariantTuple(x, ps) => {
            let p = if ps.len() == 1 {
                lower_pattern(ctx, ps[0].clone(), m)
            } else {
                lower_pattern(ctx, ast::PTuple(ps).with(info), m)
            };
...
fn lower_enum_variant(
    ...
    let (x, t) = match v {
        ...
        ast::VTuple(x, ts) => {
            if ts.len() == 1 {
                let t = lower_type(ctx, ts[0].clone());
                (x, t)
            } else {
                let ts = ts.mapm(ctx, lower_type);
    ...
    if let ast::EName(x, ts) = e.kind() {
        match ctx.stack.find_expr_decl(&x) {
            Some(ExprDecl::Variant(info1, x0, gs)) => {
                ...
                let e = if es.len() == 1 {
                    lower_expr(ctx, es[0].clone())
                } else {
                    lower_expr(ctx, ast::ETuple(es).with(info))
                };
...
fn lower_expr(ctx: &mut Context, e: ast::Expr) -> Expr {
    ...
    match e.kind() {
        ...
        ast::ELoopBreak(e) => {
            if let Some(e) = e {
                if ctx.is_inside_infinite_loop() {
                    let e = lower_expr(ctx, e);
                    ctx.typed(|t| EBreak(e).with(t, info))
                } else {
                    ctx.diagnostics
        ...
        ast::ELoopContinue => {
            if ctx.is_inside_loop() {
                ctx.typed(|t| EContinue.with(t, info))
            } else {
                ctx.diagnostics
        ...
        ast::EFun(ps, t, b) => {
            ...
            let b = lower_body(ctx, b);
            ctx.stack.pop_scope();
            ctx.typed(|t| EFun(ps, t1, b).with(t, info))
        }
        ast::EMatch(e, arms) => {
            let e = lower_expr(ctx, e);
        ...
        ast::FName(x, e) => {
            if let Some(e) = e {
                ...
                let e = match ctx.stack.find_expr_decl(&x) {
                    Some(ExprDecl::Val(_)) => ctx.typed(|t| EVar(x.clone()).with(t, info)),
...
fn pattern_to_expr_record(ctx: &mut Context, p: hir::Pattern) -> hir::Expr {
    fn f(ctx: &mut Context, p: &hir::Pattern, acc: &mut Vector<(Name, hir::Expr)>) {
        match p.kind() {
            hir::PNoop(_) => {}
            hir::PVar(x) => {
                let e = ctx.typed(|t| EVar(x.clone()).with(t, p.info));
                acc.push_back((x, e));
            }
...
fn call<const N: usize, const M: usize>(
    ...
    let e = ctx.typed(|t| EVar(x.into()).with(t, info));
