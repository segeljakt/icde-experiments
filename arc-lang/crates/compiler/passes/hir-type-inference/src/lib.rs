...
fn infer_block(ctx: &mut Context, b: Block, kind: ScopeKind) -> Block {
...
fn infer_stmt(ctx: &mut Context, s: Stmt) -> Stmt {
    ...
    match s.kind {
        ...
        SDef(m, x, gs0, ps, t, b) => {
            ...
            let b = infer_block(ctx, b, ScopeKind::Block);
...
fn infer_expr(ctx: &mut Context, e: Expr) -> Expr {
    ...
    match (*e.kind).clone() {
        ...
        ELoop(b) => {
            let b = infer_block(ctx, b, ScopeKind::Loop(e.t));
            ELoop(b).with(t, info)
        }
        EBreak(e) => {
            let e = infer_expr(ctx, e);
        ...
        EVariant(x0, ts, x1, e) => {
            ...
            unify(ctx, t1.clone(), e.t.clone(), e.info);
            EVariant(x0, ts, x1, e).with(t, info)
        }
        EIfElse(e, b0, b1) => {
            let e = infer_expr(ctx, e);
            unify(ctx, e.t.clone(), atom("Bool"), info);
            let b0 = infer_block(ctx, b0, ScopeKind::Block);
            let b1 = infer_block(ctx, b1, ScopeKind::Block);
        ...
        EWhile(e, b) => {
            let e = infer_expr(ctx, e);
            let b = infer_block(ctx, b, ScopeKind::While);
            unify(ctx, e.t.clone(), atom("Bool"), info);
...
fn infer_pattern(ctx: &mut Context, p: Pattern) -> Pattern {
    ...
    match p.kind() {
        ...
        PVariant(x0, ts, x1, p) => {
            ...
            unify(ctx, t1.clone(), p.t.clone(), p.info);
