...
fn lower_top_stmt(ctx: &mut Context, s: hir::Stmt) {
    ...
    match s.kind {
        ...
        hir::SRecDef(..) => todo!(),
        hir::SNoop => {}
        hir::SMonoDef(_, _, _, _, _, _) => todo!(),
        hir::SMonoBif(_, _, _, _, _) => todo!(),
        hir::SMonoEnum(_, _, _, _) => todo!(),
        hir::SMonoBit(_, _, _) => todo!(),
    }
}
...
fn lower_pattern(ctx: &mut Context, p: hir::Pattern) -> Val {
    ...
    match p.kind.as_ref().clone() {
        hir::PVar(x) => VName(x).with(t),
        hir::PIgnore => VIgnore.with(t),
        _ => {
            ctx.diagnostics.push_error(Error::UncompileableCode {
                info: p.info,
...
fn lower_expr_val(ctx: &mut Context, e: hir::Expr) -> Val {
    ...
    if let hir::EVar(x) = e.kind.as_ref().clone() {
        VName(x).with(t)
    } else {
        ctx.diagnostics.push_error(Error::UncompileableCode {
            info: e.info,
...
fn lower_expr_stmt(ctx: &mut Context, s: hir::Stmt) -> Stmt {
    ...
    match e.kind() {
        ...
        hir::ESsaLoop(pes, b) => {
            let vvs = pes.mapm(ctx, |ctx, (p, e)| {
                let v0 = lower_expr_val(ctx, e);
                let v1 = lower_pattern(ctx, p);
                (v0, v1)
            });
            let b = lower_block(ctx, b);
            SLoop(vvs, b).with(v)
        }
        hir::ESsaContinue(pes) => {
            let vs = pes.map(|(p, e)| {
                let v = lower_pattern(ctx, p);
