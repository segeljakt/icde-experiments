...
pub enum StmtKind {
    ...
    SMod(Meta, Name, Vector<Stmt>),
    SUse(Meta, Path, Option<UseSuffix>),
    // Internal
    SBuiltinDef(
        Meta,
        Name,
...
impl From<Spanned<StmtKind>> for Stmt {
    fn from((kind, info): Spanned<StmtKind>) -> Self {
        Self { kind, info }
    }
}

impl From<Spanned<ExprKind>> for Expr {
    fn from((kind, info): Spanned<ExprKind>) -> Self {
        Self {
            kind: Rc::new(kind),
            info,
        }
...
impl From<Spanned<TypeKind>> for Type {
    fn from((kind, info): Spanned<TypeKind>) -> Self {
        Self {
            kind: Rc::new(kind),
            info,
        }
...
impl From<Spanned<PatternKind>> for Pattern {
    fn from((kind, info): Spanned<PatternKind>) -> Self {
        Self {
            kind: Rc::new(kind),
            info,
        }
