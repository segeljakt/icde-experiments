...
#[derive(
    Clone, Debug, Send, Sync, Unpin, Serialize, Deserialize, Eq, PartialEq, Hash, Ord, PartialOrd,
)]
#[repr(C)]
...
impl<T> Vec<T> {
    ...
    {
        self.0.update(|this| {
            this.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal))
        });
        self
    }
    ...
    pub fn iter(self) -> Iter<impl Iterator<Item = T> + Clone>
    where
        T: Clone,
    {
