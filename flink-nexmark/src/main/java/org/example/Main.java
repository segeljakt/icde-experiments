...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        switch (query) {
            case "q1": {
                Query1.q1(bids(env, dir)).addSink(new DataSink<>());
                break;
            }
            case "q1-io": {
                bids(env, dir).addSink(new DataSink<>());
                break;
            }
            case "q3": {
            ...
            case "q3-io": {
                persons(env, dir).addSink(new DataSink<>());
                auctions(env, dir).addSink(new DataSink<>());
                break;
            }
            default:
    ...
    public static <T> DataStream<T> read(StreamExecutionEnvironment env, String path, Class<T> c, DataSource.TimestampExtractor<T> t, TypeInformation<T> ti) {
        return env
                .addSource(new DataSource<T>(() -> new CsvMmapIterator<T>(path, c), t, 1000, 100))
                .returns(ti);
    }
    ...
    public static DataStream<Bid> bids(StreamExecutionEnvironment env, String dir) {
        String path = dir + "/bids.csv";
        return read(env, path, Bid.class, x -> x.dateTime, TypeInformation.of(new TypeHint<Bid>() {
        }));
    }
}
