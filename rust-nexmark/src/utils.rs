...
pub fn to_bid(event: nexmark::event::Event) -> std::option::Option<Bid> {
    match event {
        nexmark::event::Event::Bid(event) => Some(Bid {
            auction: event.auction,
            bidder: event.bidder,
            price: event.price,
            channel: event.channel.into(),
            url: event.url.into(),
            date_time: event.date_time.into(),
            extra: event.extra.into(),
        }),
        _ => None,
    }
}

pub fn to_person(event: nexmark::event::Event) -> std::option::Option<Person> {
    match event {
        nexmark::event::Event::Person(event) => Some(Person {
            id: event.id,
            name: event.name.into(),
            email_address: event.email_address.into(),
            credit_card: event.credit_card.into(),
            city: event.city.into(),
...
pub fn to_auction(event: nexmark::event::Event) -> std::option::Option<Auction> {
    match event {
        nexmark::event::Event::Auction(event) => Some(Auction {
            id: event.id,
            item_name: event.item_name.into(),
            description: event.description.into(),
            initial_bid: event.initial_bid,
            reserve: event.reserve,
            date_time: event.date_time.into(),
            expires: event.expires.into(),
            seller: event.seller,
...
pub fn csv_to_vec<T: Data>(path: &str) -> std::vec::Vec<T> {
    println!("Reading records from {}", path);
...
pub fn mmap_csv<T: Data>(path: &str) -> impl Iterator<Item = T> {
    let file = std::fs::File::open(path).expect("Unable to open file");
    let mmap: memmap2::Mmap = unsafe {
    ...
    std::iter::from_fn(move || {
        ...
        if let Ok(bid) = T::deserialize(&mut deserializer) {
